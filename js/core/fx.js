/* Canvas particle effects: confetti, bubbles, sparks, plus floating XP numbers. */
...
CHEM.FX = (function () {
  ...
  const api = {
    ...
    confetti(strength) {
      ...
      for (let i = 0; i < n; i++) {
        parts.push({
          ...
          color: themeColors()[Math.floor(Math.random() * 5)],
    ...
    /** Rising bubbles — used when a titration or reaction succeeds. */
    bubbles(x, y) {
      if (reduced) return;
      for (let i = 0; i < 26; i++) {
        parts.push({
    ...
    /** Directional spark shower — used for hits in boss fights. */
    sparks(x, y, dir) {
      burst(x, y, { count: 26, angle: dir, spread: 1.6, speed: 8, size: 3,
                    life: 40, gravity: 0.35, shape: "circle" });
    },

    /** Floating "+25 XP" style text at a screen position. */
    floatText(x, y, text, color) {
      const node = CHEM.U.el("div", { class: "float-xp", text });
    ...
    /** Shake the main view — used for damage / wrong answers. */
    shake() {
      if (reduced) return;
