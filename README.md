...
include everything the syllabus does. Settings → *Course coverage* has **eleven** switches —
the whole of Year 11, plus the topics most often still untaught at trials: polymers and
polyesters, soaps and detergents, biofuels, amines and amides, NMR, mass spectrometry and
infrared, AAS and colorimetry, organic analysis and synthesis, buffers, and ocean
acidification. Switching one off removes its questions, its flashcards and its naming
entries, and drops any module left empty from the picker. In Pathway Puzzle a switched-off
topic is removed from the reaction graph itself, not just from the puzzle list — so no route
is plotted through chemistry you haven't been taught, and its reagent cards aren't dealt.
...
key is strictly longest in **24.9%** of questions — *below* the 25% you'd get by chance —
...
- **1,091** multiple-choice questions across all 8 modules, each with a worked explanation
  (M1 110 · M2 112 · M3 85 · M4 75 · M5 175 · M6 176 · M7 177 · M8 181)
...
grew from 374 to 1,091. Bigrams rather than single words, because "ΔH < 0 and ΔS > 0" and
